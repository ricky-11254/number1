 ...
 * @return array
 */
function benzin_body_classes($classes)
{
...
add_filter('body_class', 'benzin_body_classes');

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function benzin_pingback_header()
{
...
add_action('wp_print_footer_scripts', 'benzin_skip_link_focus_fix');

/**
 * Include a skip to content link at the top of the page so that users can bypass the menu.
 */
function benzin_skip_link()
{
	echo '<a class="skip-link screen-reader-text" href="#site-content">' . esc_html('Skip to the content', 'benzin') . '</a>';
}
...
if (!function_exists('benzin_primary_navigation_fallback')) :
	...
	function benzin_primary_navigation_fallback()
	{
		...
		echo '<ul>';
		echo '<li><a href="' . esc_url(home_url('/')) . '">' . esc_html__('Home', 'benzin') . '</a></li>';
		wp_list_pages(array(
			'title_li' => '',
			'depth'    => 1,
			'number'   => 5,
